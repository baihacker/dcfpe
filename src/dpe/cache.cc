...
CacheReaderImpl::CacheReaderImpl(
    const std::unordered_map<int64, VariantsReaderImpl*>& data)
    : refCount(0), data(data) {}

CacheReaderImpl::CacheReaderImpl(
    std::unordered_map<int64, VariantsReaderImpl*>&& odata)
    : refCount(0) {
  data.swap(odata);
}

CacheReaderImpl::~CacheReaderImpl() { clearMap(); }

void CacheReaderImpl::addRef() { ++refCount; }
...
CacheWriterImpl::CacheWriterImpl(const base::FilePath& path, bool reset)
    : refCount(0),
      file(path, (reset ? base::File::FLAG_CREATE_ALWAYS
                        : base::File::FLAG_OPEN_ALWAYS) |
                     base::File::FLAG_APPEND) {}

CacheWriterImpl::~CacheWriterImpl() {}

void CacheWriterImpl::addRef() { ++refCount; }
